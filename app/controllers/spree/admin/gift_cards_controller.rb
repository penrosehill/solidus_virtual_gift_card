class Spree::Admin::GiftCardsController < Spree::Admin::BaseController
  before_action :load_user, only: %i[lookup redeem]
  before_action :load_gift_card_for_redemption, only: [:redeem]
  before_action :load_gift_card_by_id, only: %i[edit update send_email deactivate]
  before_action :load_order, only: %i[edit update deactivate]
